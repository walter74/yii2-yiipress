...
if ($model_article->load(Yii::$app->request->post())) {
				/*	$nm_generic_form=$this->module->modelNamespace.'\CmsGenericFieldForm';
					...
					if(is_object($model_uploadcover->imageFile)&&$model_uploadcover->validate()){
						//$filename_url='http://'.Yii::$app->request->hostName.Yii::getAlias(Yii::$app->controller->module->upload_dir.'/'.$model_article->title .'_'.'cover_#'.$model_article->id.'_'.time(). '.' . $model_uploadcover->imageFile->extension);
					...
					if ($model_article->save()) {
								Yii::$app->session->setFlash('success','l\'articolo è stato aggiornato correttamente');
					}
					else
						Yii::$app->session->setFlash('error','Errore aggiornamento articolo!');
}
...
$this->title='Aggiorna Articolo #'.$model_article->id.' '.$model_article->title;
...
?>
<div class="col-lg-12">
					<h3 class="page-header"><i class="fa fa-plus"></i> Aggiorna Articolo</h3>
